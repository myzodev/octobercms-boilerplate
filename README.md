...
-   **Basic JavaScript & CSS structure**: Organized folder structure for custom frontend development (TypeScript in the theme).
...
-   **Prettier**: A code formatter to ensure consistent code styling across the project. (For Prettier to work correctly download [OctoberCode](https://marketplace.visualstudio.com/items?itemName=SergeyKasyanov.october-code) extension)
...
To build assets run these commands in the theme folder (e.g. `themes/theme`):
...
To include the _Vite Dev Server_ assets in your dev environment, set the `APP_ENV` variable in your `.env` file to either `local` or `production`. Once set, the _Vite Dev Server_ will automatically be included.
